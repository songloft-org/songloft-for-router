...
MODULE="songloft"
DEFAULT_BIN="/koolshare/bin/songloft"
...
nat_start_link() {
	if [ "$(dbus get songloft_enable 2>/dev/null)" = "1" ]; then
		[ ! -L "/koolshare/init.d/N99SongLoft.sh" ] && \
			ln -sf /koolshare/scripts/songloft_config.sh \
			       /koolshare/init.d/N99SongLoft.sh
	else
		[ -L "/koolshare/init.d/N99SongLoft.sh" ] && \
			rm -f /koolshare/init.d/N99SongLoft.sh >/dev/null 2>&1
	fi
}
...
start_songloft() {
	...
	_work_dir="${songloft_db_path:-/koolshare/songloft}"
